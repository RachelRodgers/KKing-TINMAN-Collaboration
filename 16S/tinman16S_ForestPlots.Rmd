...
```{r TESTCODE-forest-plot-clostridium-innocuum, eval=FALSE}


#----- Calculate the Change in Taxon Abundance Between Pre and Post -----#

clostridiumDelta <- clostridiumAbdDF %>% 
  select(unique_id, pre_post, neutropenic, Abundance) %>% 
  pivot_wider(names_from = pre_post, values_from = Abundance) %>% 
  mutate(delta_change = PRE - POST)
  #mutate(fold_change = POST/PRE, LFC = log(fold_change))

#----- Use Bootstrapping to Calculate the Avg Delta and 95% CI per Group -----#

# neutropenic samples:
clostridiumNeutropenic <- clostridiumDelta %>% 
  filter(neutropenic == "Neutropenic")

set.seed(14889841)
bootResultNeutropenic <- boot::boot(clostridiumNeutropenic$delta_change,
                                    statistic, R = 1000)
neutroClostridiumMeanDelta <- base::mean(bootResultNeutropenic$t)
set.seed(14889841)
neutroClostridiumCI <- boot.ci(bootResultNeutropenic, type = "basic")
# printing neutroClostridiumCI returns the bounds for the "basic" 95% CI
# here lower and upper are -166.8 and 1063.3
# compare to t-test -242 and 1400
clostridiumNeutroFinal <- data.frame("category" = "Neutropenic",
                                     "n" = nrow(clostridiumNeutropenic),
                                     "mean" = neutroClostridiumMeanDelta,
                                     "lower" = neutroClostridiumCI$basic[1, 4],
                                     "upper" = neutroClostridiumCI$basic[1, 5])

# control samples:
clostridiumControl <- clostridiumDelta %>% 
  filter(neutropenic == "Control")

set.seed(14889841)
bootResultControl <- boot::boot(clostridiumControl$delta_change,
                                statistic, R = 1000)
controlClostridiumMeanDelta <- base::mean(bootResultControl$t)
# here mean is -413.7854, compared to t-test -404
set.seed(14889841)
controlClostridiumCI <- boot.ci(bootResultControl, type = "basic")
# here lower and upper are -924.6 and 177.2 compared to -951 and 141
clostridiumControlFinal <- data.frame("category" = "Control",
                                      "n" = nrow(clostridiumControl),
                                      "mean" = controlClostridiumMeanDelta,
                                      "lower" = -924.6,
                                      "upper" = 177.2)

#----- Combine Data for Middle Portion of the Plot -----#

clostridiumForestData <- rbind(clostridiumNeutroFinal, clostridiumControlFinal) %>% 
  mutate(across(c(mean, lower, upper), round),
         estimate_lab = paste0(mean, " (", lower, "-", upper, ")"))

clostridiumPlotMiddle <- ggplot(clostridiumForestData,
                                aes(y = category)) +
  theme_classic() +
  geom_point(aes(x = mean, size = n), shape = 15) +
  scale_size_continuous(range = c(3, 6)) +
  ...
  annotate("text", x = -500, y = 2.5, label = "Control", size = 4) +
  annotate("text", x = 750, y = 2.5, label = "Neutropenic", size = 4) +
  #ggtitle("Average Change in Abundance of Clostridium innoccum") +
  #labs(x = "Change in Abundance (Pre - Post) and 95% CI", y = "Group") +
  theme(legend.position = "none",
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 14),
        plot.title = element_text(hjust = 0.5, size = 14),
        ...
        axis.text.y= element_blank(),
        axis.title.y= element_blank())
clostridiumPlotMiddle

#----- Plot Left Side of Plot -----#

# Add a row of data to the data frame that is just column names shown on plot
clostridiumForestTitles <- data.frame(category = "Category", n = "", mean = "",
                                      lower = "", upper = "", 
                                      estimate_lab = "Avg. Change (95% CI)")
clostridiumForestDataLeft <- rbind(clostridiumForestData, 
                                   clostridiumForestTitles)
clostridiumForestDataLeft$category <- factor(clostridiumForestDataLeft$category,
                                             levels = c("Control",
                                                        "Neutropenic",
                                                        "Category"))

clostridiumPlotLeft <- ggplot(clostridiumForestDataLeft,
                              aes(y = category)) +
  geom_text(aes(x = 0, label = category), hjust = 0, fontface = "bold") +
  geom_text(aes(x = 1, label = estimate_lab),  hjust = 0,
            fontface = ifelse(clostridiumForestDataLeft$estimate_lab == "Avg. Change (95% CI)",
                              yes = "bold", no = "plain")) +
  theme_void() +
  coord_cartesian(xlim = c(0, 3), ylim = c(0, 3))
clostridiumPlotLeft

#----- Merge the Left and Middle Panels for the Clostridium Plot -----#

clostridiumForestFinal <- c(
  area(t = 0, l = 0, b = 30, r = 3),
  area(t = 1, l = 4, b = 30, r = 9)
)

clostridiumPlotLeft + clostridiumPlotMiddle + plot_layout(design = clostridiumForestFinal)
```