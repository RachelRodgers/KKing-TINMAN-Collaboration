...
```{r t-test-code, include=FALSE, eval=FALSE}
neutroMean <- base::mean(clostridiumNeutropenic$delta_change) 
neutroCI <- t.test(clostridiumNeutropenic$delta_change)
neutroLower <- neutroCI$conf.int[1]
neutroUpper <- neutroCI$conf.int[2]
...
controlMean <- base::mean(clostridiumControl$delta_change)
controlCI <- t.test(clostridiumControl$delta_change)
controlLower <- controlCI$conf.int[1]
controlUpper <- controlCI$conf.int[2]
...
clostridiumFPDataFinal <- rbind(clostridiumNeutroFinal,
                                clostridiumControlFinal)

plot <- ggplot(clostridiumFPDataFinal,
               aes(y = category)) +
  theme_classic() +
  ...
  labs(x = "Change in Abundance (Pre - Post) and 95% CI",
       y = "Group") +
  theme(legend.position = "none",
