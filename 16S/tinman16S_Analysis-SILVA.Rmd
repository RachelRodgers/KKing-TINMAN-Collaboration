...
and comparing to matched controls. Each patient has a "pre" and "post" time 
point. This data set will be analyzed in several different groupings:
...
```{r remove-retrospective-neutropenic-samples}
# As per request on May 28, 2024, remove the neutropenic samples originating
#   from the retrospective arm
removeSamples <- c("053A2", "053D2", "061A2", 
                   "061D2", "069C2", "069D2", 
                   "070B2", "070D2")
sampleDataFiltered <- sampleDataFinal %>% 
  filter(!sample_name %in% removeSamples)
```

```{r physeqMerged, results='hide'}
physeqMerged <- merge_phyloseq(physeqRaw, sample_data(sampleDataFiltered))
...
names(tpCndColors) <- unique(sampleDataFiltered$timepoint_condition)
...
```{r sampleOverview}
sampleOverview <- sampleDataFiltered %>% 
...
readsPerSampleDF <- sampleDataFiltered %>% 
...
Reads/sample range from 1,521 to 45,885 with an average of 28,874.
...
```
```{r readDistBox}
readDistBox <- PrettyBox(df = sampleDataFiltered,
...
```{r readDistBoxByTime}
readDistBoxByTime <- PrettyBox(df = sampleDataFiltered,
                               xVar = "neutropenic", yVar = "readsPerSample",
                               statMethod = "wilcox.test",
...
readDistBoxByGroup <- PrettyBox(df = sampleDataFiltered,
...
communityCompXLabs <- sampleDataFiltered %>% 
...
```{r alphaCombinedTP, fig.width=9, fig.height=6}
...
control samples when using both UniFrac and weighted UniFrac.
